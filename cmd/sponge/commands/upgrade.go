...
import (
	...
	"github.com/i2dou/sponge/pkg/gobash"
...
func runUpgradeCommand() error {
	...
	result := gobash.Run(ctx, "go", "install", "github.com/i2dou/sponge/cmd/sponge@latest")
...
func copyToTempDir() (string, error) {
	...
	arg := fmt.Sprintf("%s/pkg/mod/github.com/i2dou", gopath)
	...
	if latestSpongeDirName == "" {
		return "", fmt.Errorf("not found sponge directory in '$GOPATH/pkg/mod/github.com/i2dou'")
	}

	srcDir := adaptPathDelimiter(fmt.Sprintf("%s/pkg/mod/github.com/i2dou/%s", gopath, latestSpongeDirName))
...
func updateSpongeInternalPlugin(latestVersionNum string) {
	...
	result := gobash.Run(ctx, "go", "install", "github.com/i2dou/sponge/cmd/protoc-gen-go-gin@"+latestVersionNum)
	for v := range result.StdOut {
		_ = v
	}
	if result.Err != nil {
	...
	ctx, _ = context.WithTimeout(context.Background(), time.Minute) //nolint
	result = gobash.Run(ctx, "go", "install", "github.com/i2dou/sponge/cmd/protoc-gen-go-rpc-tmpl@"+latestVersionNum)
	for v := range result.StdOut {
		_ = v
	}
	if result.Err != nil {
